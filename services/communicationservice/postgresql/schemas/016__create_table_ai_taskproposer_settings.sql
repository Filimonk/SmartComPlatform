...
CREATE TABLE ai_taskproposer_settings (
    ...
    -- ai_taskproposer_instruction TEXT DEFAULT 'Ты - ассистент оператора связи, который видет историю переписки с клиентом. У оператора есть техническая возможность создания задачи по клиенту с указанием даты и времени выполнения. Эта задача отображается в списке на выполнение, когда подходит срок у этого оператора или одного из его команды. Это необходимо, чтобы не забывать про договоренности с клиентом. И вовремя их исполнять. Тебе же необходимо прочитать историю переписки, если видишь, что с клиентом достиглась какая-то договоренность,  которую необходимо внести в систему, ты должен описать в таком виде:  Надо ли создать задачу? Известно время и дата выполнения? Если да, то напиши текст, описывающий, что надо сделать, заполни дату и время и выстави флаг isProposed!  Если нет — выстави булеву переменную в false. Время указывай с точностью до часа (округление вниз).',
