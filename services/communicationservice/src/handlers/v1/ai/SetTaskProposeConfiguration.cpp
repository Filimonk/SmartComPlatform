#include "SetTaskProposeConfiguration.hpp"
...
SetTaskProposeConfiguration::SetTaskProposeConfiguration(const userver::components::ComponentConfig& config,
...
auto SetTaskProposeConfiguration::HandleRequestJsonThrow(const userver::server::http::HttpRequest& request,
    ...
    try {
        ...
        const auto request_dto = request_json.As<dto::SetTaskProposeConfigurationRequest>();

        const auto& result = pg_cluster_->Execute(
            userver::storages::postgres::ClusterHostType::kMaster,
            communicationservice::sql::kSetTaskProposeConfiguration, user_id, request_dto.url, request_dto.timeout, request_dto.instruction);

        if (result.IsEmpty()) {
            LOG_ERROR() << "The user's organization not found";
