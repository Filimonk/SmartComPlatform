#include "GetTaskProposeConfiguration.hpp"
...
GetTaskProposeConfiguration::GetTaskProposeConfiguration(const userver::components::ComponentConfig& config,
...
auto GetTaskProposeConfiguration::HandleRequestJsonThrow(const userver::server::http::HttpRequest& request,
    ...
    try {
        ...
        const auto& result = pg_cluster_->Execute(
            ...
            communicationservice::sql::kGetAiTaskproposerSettings, user_id);

        if (result.IsEmpty()) {
            LOG_ERROR() << "The user's ai configuration not found";

            throw userver::server::handlers::InternalServerError(
                ErrorBuilder{dto::ErrorCode::kInternalError, "Internal Server Error"});
        }
        
        dto::GetTaskProposeConfigurationResponse response;
        response.url = result[0]["ai_taskproposer_url"].As<std::string>();
        response.timeout = result[0]["ai_taskproposer_timeout"].As<std::chrono::seconds>().count();
        response.instruction = result[0]["ai_taskproposer_instruction"].As<std::string>();
