#include "GetSpellCheckConfiguration.hpp"
...
GetSpellCheckConfiguration::GetSpellCheckConfiguration(const userver::components::ComponentConfig& config,
...
auto GetSpellCheckConfiguration::HandleRequestJsonThrow(const userver::server::http::HttpRequest& request,
    ...
    try {
        ...
        dto::GetSpellCheckConfigurationResponse response;
        response.url = result[0]["ai_spellchecker_url"].As<std::string>();
        response.timeout = result[0]["ai_spellchecker_timeout"].As<std::chrono::seconds>().count();
        response.instruction = result[0]["ai_spellchecker_instruction"].As<std::string>();
