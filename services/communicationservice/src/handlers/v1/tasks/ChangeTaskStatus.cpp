...
ChangeTaskStatus::ChangeTaskStatus(const userver::components::ComponentConfig& config,
...
auto ChangeTaskStatus::HandleRequestJsonThrow(
    ...
    try {
        ...
        const auto task_id = userver::utils::BoostUuidFromString(request.GetPathArg("taskId"));
        const auto request_dto = request_json.As<dto::ChangeTaskStatusRequest>();

        const auto& result = pg_cluster_->Execute(
            userver::storages::postgres::ClusterHostType::kMaster,
            communicationservice::sql::kChangeTaskStatus, user_id, task_id, dto::ToString(request_dto.status));

        if (result.IsEmpty()) {
            throw userver::server::handlers::ResourceNotFound(ErrorBuilder{
                dto::ErrorCode::kResourceNotFound, "Resource Not Found. Timeout exceeded"});
        }

        request.SetResponseStatus(userver::server::http::HttpStatus::kNoContent); // 204
