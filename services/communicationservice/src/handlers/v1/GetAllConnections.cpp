...
auto GetAllConnections::HandleRequestJsonThrow(
    ...
    try {
        ...
        const auto& result = pg_cluster_->Execute(
            ...
            communicationservice::sql::kGetAllConnectionsByContact,
            user_id,
            contact_id);

        dto::GetAllConnectionsResponse response;

        std::vector<dto::GetAllConnectionsResponse::ConnectionsA> connections;

        for (const auto& row : result) {
            dto::GetAllConnectionsResponse::ConnectionsA item;

            item.id = row["id"].As<boost::uuids::uuid>();
            item.contactId = row["contact_id"].As<boost::uuids::uuid>();
